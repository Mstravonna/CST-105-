  ...
  for (Object object: currentRoster) { // List all defensive players
   if (((NFLPlayer) object).getCategory() == "off") {
    ...
    i++;
         ...
         // Set grid headers
         defResults = currentRoster.stream().filter((object) -> (((NFLPlayer) object).getCategory().equals("def"))).map((_item) -> 1).reduce(defResults, Integer::sum); // List all defensive players
